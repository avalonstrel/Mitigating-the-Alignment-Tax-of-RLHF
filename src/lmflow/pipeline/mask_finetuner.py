...
from lmflow.pipeline.utils.mask_trainer import MaskTrainer

logger = logging.getLogger(__name__)


class MaskFinetuner(BaseTuner):
    """
    ...
    def tune(self, model0, model1, dataset, transform_dataset_in_place=True):
        """
        ...
        trainer = MaskTrainer(
            model0=model0.get_backend_model(),
            model1=model1.get_backend_model(),
            args=training_args,
        ...
        # print('aaa', self.finetuner_args)
        # if self.finetuner_args.kl_weight > 0 and isinstance(FinetuningTrainer, RaftTrainer):
        #     trainer.penalty_init()
